...
Compose)、VLM 本體(llama.cpp;core-rs 只透過 FFI 介面呼叫)。
...
- **llama.cpp**(via Rust FFI)—— L1 VLM 推論(被 L0 閘控後才呼叫)。
...
- **FR-1(P1,已起步)** L0 變化閘控:以 aHash + Hamming 距離判斷畫面是否改變到值得叫 L1,
  **省算力**(靜態場景不叫 VLM)。
- **FR-2(P2)** 影格管線協調:閘控 → L1(llama.cpp)→ 產生 Kineme。
...
- L1 由 llama.cpp 提供(FFI);L2 型別對齊 `schemas/event.schema.json`(待建)。
